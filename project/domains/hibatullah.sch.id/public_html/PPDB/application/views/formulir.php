                    ...
                    <div class="h-100 d-flex flex-column justify-content-center p-2">
                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="form-group">
                                <label for="gProgram">Unit / Cabang</label>
                                <select name="program" id="gProgram" class="form-control bg-white" required>
                                    <?php if (count($program->data) > 1) : ?>
                                        <option value="">- Pilih -</option>
                                    <?php endif; ?>
                                    <?php foreach ($program->data as $prg) : ?>
                                        <option value="<?= $prg->id ?>" data-cabang="<?= $prg->nama_cabang ?>"><?= $prg->nama_cabang ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <input type="hidden" class="form-control Cabang" name="Cabang" value="">
                            </div>
                        ...
                        <div class="DataSet" style="display: none;">
                            <div class=" g-2  row">
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        <label for="gPeriode">Periode Pendaftaran</label>
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label for="gKota_lahir">Tempat Lahir</label>
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <input type="text" class="form-control" id="cIbu" placeholder="Nama Ibu" name="ibu" required>
                                        <label for="cIbu">Nama Ibu</label>
                                    </div>
                                </div> -->
                                <div class="col-sm-6">
                                    <div class="form-group">
...
<script>
    var url = '<?= base_url() ?>';
    $("#gProgram").change(function() {
        var id = $(this).val();
        var cbid = $(this).find(':selected').data('cabang');
        $(".Cabang").val(cbid);
        $.ajax({
            url: url + 'formulir/get_periode/' + id,
            dataType: 'json',
            error: function(x) {
                alert("ERROR");
            ...
            success: function(data) {
                ...
                if (data.error != true) {
                    ...
                    $.each(data.data, function(i, val) {
                        periode += `<option value="${val.tahun_akademik}" data-nominal="${val.nominal}">${val.tahun_akademik}</option>`;
                    });

                    $("#gPeriode").html(periode);
                    $(".DataSet").css("display", "block");
                    $("#ReportPddb").html("");
                } else {
                    $(".DataSet").css("display", "none");
                    $("#ReportPddb").html(`<h6 id="" class="my-2 px-4 py-2 bg-light rounded text-center">${data.data}</h6>`);
                }
            }
        })
    });
