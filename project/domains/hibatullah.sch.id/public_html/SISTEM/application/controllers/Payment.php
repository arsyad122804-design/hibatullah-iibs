...
class Payment extends CI_Controller
{
    ...
    public function token()
    {
        ...
        $tokenset = $this->input->post('tokenset');

        $params = json_decode($this->secure->decrypt_url($tokenset));
        ...
        echo $snapToken;
        if ($snapToken) {
            $ceckingdata = $this->db->get_where("master_pmb_reg", ['nomor_reg' => $noregister, 'jns' => $jns]);

            if ($ceckingdata->num_rows() > 0) {
                $Updates = [
                    'pembayaran'        => $setpmb,
                    'nota'              => $order_id,
                ...
                $this->db->where("nomor_reg", $noregister);
                $this->db->where("jns", $jns);
                $this->db->update("master_pmb_reg", $Updates);
            } else {
                $saves = [
                ...
                $this->db->insert("master_pmb_reg", $saves);
            }
        }
    }
