...
class Pendaftar extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->lang->load('auth');
        $this->load->model(['affiliate/m_pendaftar', 'm_lembaga']);
    ...
    function get_data($ta_set = null, $aff_set = null)
    {
        ...
        // if ($lvl == 1) {
        //     $data =  $this->m_pendaftar->GetAllDataUser()->result();
        // } else {
        //     $data =  $this->m_pendaftar->GetAllDataUserID($ids)->result();
        // }
        ...
        if ($lvl == 1) {
            $data =  $this->m_pendaftar->GetAllDataUserByTa($ta)->result();
        } else {
            $data =  $this->m_pendaftar->GetAllDataUserIDBy($ids, $ta)->result();
        }
        if ($aff_set != null) {
            if ($aff_set != 'null') {
                if ($aff_set != 'all') {
                    if ($aff_set != 'undefined') {
                        $data =  $this->m_pendaftar->GetAllDataUserIDBy($aff_set, $ta)->result();
                    }
                }
            }
        }
